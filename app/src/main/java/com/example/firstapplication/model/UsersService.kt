...
class UsersService {
    ...
    fun load(newUsers: List<User>) {
        users = (1..newUsers.count()).map { User(
            id = it.toLong(),
    ...
    {
        val indexToChange = users.indexOfFirst { it.id == user.id }
        deleteUser(users[indexToChange])
        users.add(user)
        notifyChanges()
    }
